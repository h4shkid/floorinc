"""
Fast NetSuite -> Local SQLite -> Cloudflare R2 sync.

Phase 1: Fetch from NetSuite -> write to local SQLite (fast local disk I/O)
Phase 2: Upload entire .db file to R2 (~5 seconds)

Performance:
  - NetSuite fetch: ~15 min (API-bound)
  - Local SQLite writes: <1 sec
  - R2 upload: ~5 sec (vs hours with row-by-row Turso push)

Run: cd backend && python3 push_to_r2.py
"""
...
# --- R2 credentials (fill these in from Cloudflare dashboard) ---
os.environ["R2_ACCOUNT_ID"] = "f3f0bcf6b12fc837cffa9a9e326565c8"
os.environ["R2_ACCESS_KEY_ID"] = "d649d7dc01bb629f2b4ab0168c86d7ce"
os.environ["R2_SECRET_ACCESS_KEY"] = "4d764c7cb89ad3ee7c251c5fc85767c00c2f8c1ec70cb1bea578cd39506ad0ce"
os.environ["R2_BUCKET"] = "flooringinc"
...
def main():
    ...
    # ==========================================
    # PHASE 2: Upload to R2 (~5 seconds)
    # ==========================================
    log("")
    log("=== PHASE 3: UPLOAD TO CLOUDFLARE R2 ===")
    upload_db(LOCAL_DB)
